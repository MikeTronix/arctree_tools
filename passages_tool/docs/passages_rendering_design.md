...
**Openings / niches / volumes:** a non-billboard arch with `kind=opening` or `depth_m>0` is a 3D slab (profile `rect`/`round`/`gothic`, wall punch, n-slice skipped). `kind=niche` stays a planar span on the wall for POM. `kind=volume` is an additive pilaster box extruded into the room (five faces; wall not punched; Kind Volume applies 0.55 × 0.45 m if width was still door-sized). `kind=recess` punches and boxes inward. On a closed wall, extrusion uses the room interior even if the arch azimuth is 180° off (open walls keep the authored angle). `alpha_blend` and billboard stay cards.
