...
| `Level.clear_wall_png_overrides` | `editor/level.py` | Drops interval PNG filenames so `meta.style` band compose dresses the wall. Properties **Use style on this wall**. |
...
11. **Turn slew from a per-node yaw strip (client + baker).** **done (compat).** Baker writes extra `yaw_vXXXX.png` (four 90° faces, clockwise unwrap 0/270/180/90 so Panda image-left = +yaw seams match; pixel 0 is yaw +45°). Manifest `eyepoints.*.yaw_strip` is **optional**. Client pans a `fov_h` window (80–400 ms by |Δyaw|) then lands on the hi-res still; missing strip ⇒ still-to-still crossfade. Left turn decreases strip x (new scenery from the left). Dual copies wrap the 45° seam. Re-bake strips after the CW unwrap (`--force`). A forward step onto a bent corridor (e.g. 90°) slews at the arrival vertex. `--no-yaw-strip` skips extras. Do not replace sharp viewpoints with the strip.
